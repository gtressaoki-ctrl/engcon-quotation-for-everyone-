...
> ⚠️ **本書は初期要件定義書（v1.0）のアーカイブであり、現在の実装と乖離している。**
> 現状の正は `CLAUDE.md`（運用ルール）と `docs/architecture.md`（現状アーキテクチャ。
> README との乖離一覧付き）。設計判断は `docs/adr/`、未実装仕様は `docs/specs/` を参照。
> 特に **§7 の PDF 出力は「実装しない」ことが確定済み**（docs/adr/0002）。
> 見積番号形式・reseller 掛け率・PWA 方式なども本書の記述から変わっている。
...
> ⚠️ **本節は実装しないことが確定した（2026-07-05 ユーザー決定、docs/adr/0002）。**
> 見積のアウトプットは DB 保存＋メール通知＋管理ダッシュボード（一覧/CSV/改訂）で完結する。
> 以下は歴史的記録として残す。
