...
> **※ 現在この機能は実装していません（2026年8月時点）。**
> PDF出力は見送りとし、STEP10（確認・出力）では見積データの保存のみを行います。
> 以下の仕様は当初の要件として残していますが、実装対象ではありません。
> 関連の未使用ファイル（components/pdf/QuoteTemplate.ts、vercel.json の
> generate-pdf 設定）は削除済みです。
