...
- プルダウン候補（既存ディーラーリスト）：
  生振商会 / IB / 寿 / シーエン / 原商 / ギアトライム / 喜多機械 / サーデック / ヤマノ / 下元
  ※後から管理画面で追加・削除できること
...
- 機種名（テキスト入力）
- 製造年月（任意・テキスト入力）
- キャビン仕様確認チェック（必須・チェックしないと次へ進めない）
...
| S45 | EC206 または EC209（ユーザーが選択） |
| S60 | EC214 または EC219（ユーザーが選択） |
| S70 | EC226 または EC233（ユーザーが選択） |
| S80 | EC233（固定） |
...
2. クイックカプラ（S規格対応品）
3. グリッパー（S規格対応品）
4. DC2コントロールシステム/ジョイスティック
5. QSCシステム（EXTDC2-MAP30-QH5）
6. ホースプロテクション × 4本（単価2,200円、下2桁四捨五入適用）
