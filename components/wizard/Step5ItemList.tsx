...
export default function Step5ItemList() {
  ...
  async function buildDefaultItems() {
    ...
    // 標準構成（品番・数量）は lib/standardItems.ts に集約。ここでは価格・正式品名を引き当てる。
    const { specs, noteAdditions } = buildStandardItemSpecs({
      machine_maker, machine_model, mount_type, s_standard, ec_model, dc_system, client_type,
    });

    const built: QuoteItem[] = [];
    for (const spec of specs) {
      const r = spec.item_no ? await lookup(spec.item_no) : {};
      built.push(makeItem(spec.fallback_name, r.price, price_type, spec.item_no, spec.qty, r.description));
    }
